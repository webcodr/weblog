...
- `rg` or ripgrep is `grep` on steroids, respecting git ignore files and skipping binaries or hidden files
...
`rgp something` -- this will use `rg` to recursively search the current directory for files with `something` inside. The result is piped into `fzf`. You can now filter the result further with `fzf` or select a file with the arrow keys and `bat` will give you a live preview of the file with syntax highlighting. Pressing `enter` will open the selected file with your default editor (environment variable `EDITOR`).

`fdp` works similarly, but looks for file names instead of the file content and also pipes the result to `fzf` for further filtering, preview or opening the selected file.

Don't like fish? Here's a version that works with bash or zsh. 
