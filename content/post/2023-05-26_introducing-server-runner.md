...
---
In my recent adventures with Rust, I planned to write a REST API with the help of the excellent book "Zero To Production In Rust" from Luca Palmieri. That's still happening, but as a small side project, I wanted to write some kind of CLI tool.
...
But I wanted to do this properly and so I decided to rewrite this script as a small CLI tool program in Rust: Server Runner. Well, not a very creative name, but it does what it says.
...
Server Runner is available as a [Cargo Crate](https://crates.io/crates/server-runner) and will be published soon on NPM with executables for macOS (aarch64, x86_64), Linux (aarch64, x86_64) and Windows (x86_64).
