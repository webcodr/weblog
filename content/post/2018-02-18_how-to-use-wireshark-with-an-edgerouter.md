...
~~~ sh
ssh user@edgerouter_ip 'sudo tcpdump -f -i eth0 -w -' | wireshark -k -i -
~~~

If you're monitoring the interface with your SSH connection to the EdgeRouter, you may want to ignore traffic on port 22.

~~~ sh
ssh user@edgerouter_ip 'sudo tcpdump -f -i eth1 -w - not port 22' | wireshark -k -i -
~~~
...
~~~ sh
ssh user@edgerouter_ip "sudo tcpdump -f -i eth0 -w -" | "C:\Program Files\Wireshark\Wireshark.exe" -k -i -
~~~
...
- `CTRL + C` or `CTRL + X` will not work to terminate the SSH connection. You have to close the window instead.
